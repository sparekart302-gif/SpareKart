...
export function ProductCard({ product, compact = false }: { product: Product; compact?: boolean }) {
  ...
  const seller = state.sellersDirectory.find((item) => item.slug === product.sellerSlug) ?? getSeller(product.sellerSlug);
  ...
  if (compact) {
    return (
      <div className="group relative flex h-full flex-col overflow-hidden rounded-xl bg-card shadow-[var(--shadow-soft)] transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-elevated)] sm:rounded-2xl">
        {product.badge && (
          <span className={`absolute left-2 top-2 z-10 rounded-full px-1.5 py-0.5 text-[8px] font-bold uppercase tracking-wider sm:left-2.5 sm:top-2.5 sm:px-2 sm:py-1 sm:text-[9px] ${
        ...
        <Link to="/product/$slug" params={{ slug: product.slug }} className="relative block aspect-square overflow-hidden bg-surface">
          <OptimizedImage
        ...
        <div className="flex flex-1 flex-col p-2.5 sm:p-3">
          ...
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-[15px] font-black text-foreground tabular-nums sm:text-base">{formatPKR(product.price)}</span>
            {product.comparePrice && (
              <span className="text-[11px] text-muted-foreground line-through tabular-nums">{formatPKR(product.comparePrice)}</span>
            )}
          </div>

          <div className="mt-2 text-[11px]">
            <span className={`font-medium ${stockState === "in" ? "text-success" : stockState === "low" ? "text-warning-foreground" : "text-destructive"}`}>
              {stockState === "in" ? "In stock" : stockState === "low" ? `Only ${availableStock} left` : "Out of stock"}
            </span>
          </div>

          <div className="mt-auto pt-2.5 sm:pt-3">
            <div className="mb-2 text-[11px] text-muted-foreground truncate">
              by <Link to="/seller/$slug" params={{ slug: seller.slug }} className="font-semibold text-foreground hover:text-accent">{seller.name}</Link>
              {seller.verified && <BadgeCheck className="ml-1 inline h-3.5 w-3.5 text-info" />}
            </div>
            <button
              onClick={handleAddToCart}
              disabled={availableStock === 0}
              title={!canAddToCart ? "Shopping cart is only for customers or guest checkout" : undefined}
  ...
  return (
    <div className="group relative flex h-full flex-col overflow-hidden rounded-[22px] bg-card shadow-[var(--shadow-soft)] transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-elevated)] sm:rounded-[28px]">
      {product.badge && (
        <span className={`absolute left-2.5 top-2.5 z-10 rounded-full px-2 py-0.5 text-[9px] font-bold uppercase tracking-wider sm:left-3 sm:top-3 sm:px-2.5 sm:py-1 sm:text-[10px] ${
          product.badge === "deal" ? "bg-accent text-primary"
          : product.badge === "best-seller" ? "bg-primary text-primary-foreground"
          : product.badge === "new" ? "bg-success text-success-foreground"
          : "bg-info text-white"
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-4">
        ...
        <div className="mt-3 flex items-center justify-between gap-3 text-xs">
          ...
          <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 font-medium ${
            stockState === "in"
            ...
            {stockState === "in" ? "In stock" : stockState === "low" ? `${availableStock} left` : "Out"}
          </span>
        </div>

        <Link to="/seller/$slug" params={{ slug: seller.slug }} className="mt-2.5 flex items-center gap-2 rounded-xl bg-surface px-2.5 py-2 transition-colors hover:bg-accent-soft sm:mt-3 sm:rounded-2xl sm:py-2.5">
          <OptimizedImage src={seller.logo} alt={seller.name} width={32} height={32} className="h-8 w-8 rounded-xl object-cover" />
          <div className="min-w-0 flex-1">
            ...
            <div className="text-[11px] text-muted-foreground">{seller.city}</div>
          </div>
        </Link>
        ...
        <div className="mt-auto pt-3 sm:pt-4">
          <div className="flex items-end justify-between gap-2.5 border-t border-border pt-3 sm:gap-3 sm:pt-4">
            <div>
              <div className="text-base font-black text-foreground tabular-nums sm:text-lg">{formatPKR(product.price)}</div>
              {product.comparePrice && (
                <div className="text-xs text-muted-foreground line-through tabular-nums">{formatPKR(product.comparePrice)}</div>
              )}
            </div>
            <button
              onClick={handleAddToCart}
