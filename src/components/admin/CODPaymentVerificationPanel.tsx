...
export function CODPaymentVerificationPanel({
  ...
  const verificationRows = useMemo<PaymentVerificationRow[]>(() => {
    return orders
      ...
      .filter((row) => {
        const searchable =
          `${row!.order.orderNumber} ${row!.customerName} ${row!.customerPhone}`.toLowerCase();
  ...
  const selectedProofAttempts = selectedRow
    ? getProofAttemptsForOrder(state, selectedRow.order.id)
    : [];
  ...
  useEffect(() => {
    ...
    setDraft({
      ...
      amountPaid: activeProof?.amountPaid
        ? String(activeProof.amountPaid)
        : String(selectedRow.order.totals.total),
    ...
    setActiveTab(
      activeProof
        ? selectedRow.payment.status === "UNDER_REVIEW"
          ? "REVIEW"
          : "HISTORY"
        : "CAPTURE",
    );
  ...
  const underReviewCount = verificationRows.filter(
    (row) => row.payment.status === "UNDER_REVIEW",
  ).length;
  ...
  const handleFileUpload = async (file?: File) => {
    ...
    try {
      ...
      const asset = await uploadImageAsset({
        file,
        kind: "payment-proof",
        ownerHint: selectedRow?.order.orderNumber,
      });
      ...
      toast.error(error instanceof Error ? error.message : "Unable to upload COD proof.");
    } finally {
  ...
  const handleSubmitProof = async () => {
    ...
    try {
      await submitCODProof(selectedRow.order.id, {
  ...
  const handleReviewProof = async (decision: "APPROVE" | "REJECT") => {
    ...
    try {
      if (decision === "APPROVE") {
        await approveProof({ proofId: selectedRow.activeProof.id, adminNote });
        toast.success("COD payment approved.");
        return;
      }

      await rejectProof({ proofId: selectedRow.activeProof.id, adminNote });
      ...
      <div className="space-y-4">
        ...
        <section className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <AdminCompactStat
            ...
            helper="Cash booked successfully"
            tone="success"
          />
          <AdminCompactStat
            label="Rejected"
            value={String(rejectedCount)}
            helper="Needs a fresh proof"
            tone="danger"
          />
        </section>
        ...
        <section className="grid gap-4 xl:grid-cols-[minmax(0,0.78fr)_minmax(0,1.22fr)]">
            ...
            <div className="mt-3 -mx-3 sm:-mx-4">
              {verificationRows.length === 0 ? (
                <div className="px-3 pb-1 pt-1 sm:px-4">
                  <AdminEmptyState
                    title="No COD settlements found"
                    body="There are no COD orders available in the current filter."
                  />
                </div>
              ) : (
                  ...
                  {verificationRows.map((row) => (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            {row.activeProof ? (
                              <ProofStatusBadge status={row.activeProof.status} />
                            ) : null}
                          </div>
                      ...
                      <div className="mt-2 flex flex-wrap gap-2">
                        <AdminPill tone="info">
                          {row.activeProof
                            ? `Attempt ${row.activeProof.attemptNumber}`
                            : "Capture receipt"}
                        </AdminPill>
                        {row.activeProof?.transactionReference ? (
                          <AdminPill>{row.activeProof.transactionReference}</AdminPill>
                        ) : null}
                      </div>
                    </button>
                  ))}
          ...
          <AdminPanel
            ...
            description="Keep the full decision flow in one focused workspace."
            className="h-fit"
          >
            {!selectedRow ? (
              <AdminEmptyState
                title="Select a COD order"
                body="Choose a row from the queue to capture or review its settlement proof."
              />
            ) : (
              <div>
                <div className="flex flex-wrap items-start justify-between gap-3 border-b border-border/60 pb-3">
                  <div className="min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {selectedRow.activeProof ? (
                        <ProofStatusBadge status={selectedRow.activeProof.status} />
                      ) : null}
                    </div>
                  ...
                  <AdminPill
                    tone={selectedRow.order.status === "DELIVERED" ? "success" : "warning"}
                  >
                ...
                <div className="grid gap-3 border-b border-border/60 py-3 sm:grid-cols-2 xl:grid-cols-4">
                  <CompactMeta
                    ...
                    value={selectedRow.payment.status.replaceAll("_", " ")}
                  />
                  <CompactMeta
                    label="Customer due"
                    value={formatPKR(selectedRow.order.totals.total)}
                  />
                  <CompactMeta
                    label="Seller count"
                    value={String(selectedRow.order.sellerFulfillments.length)}
                  />
                      ...
                      <CompactField label="Delivery partner">
                        <input
                          ...
                          onChange={(event) =>
                            setDraft((previous) => ({
                              ...previous,
                              deliveryPartnerName: event.target.value,
                            }))
                          }
                      ...
                      <CompactField label="Partner phone">
                        <input
                          ...
                          onChange={(event) =>
                            setDraft((previous) => ({
                              ...previous,
                              deliveryPartnerPhone: event.target.value,
                            }))
                          }
                          className="h-10 w-full rounded-xl border border-border/60 bg-surface px-3 text-sm focus:outline-none"
                        />
                    ...
                    <div className="space-y-3">
                      <div className="grid gap-3 rounded-[18px] border border-border/60 bg-surface p-3 sm:grid-cols-2">
                        <CompactMeta
                          ...
                          value={selectedRow.activeProof?.deliveryPartnerName ?? "Not added"}
                        />
                        <CompactMeta
                          label="Partner phone"
                          value={selectedRow.activeProof?.deliveryPartnerPhone ?? "Not added"}
                        />
                        <CompactMeta
                          label="Reference"
                          value={selectedRow.activeProof?.transactionReference ?? "Not added"}
                        />
                        <CompactMeta
                          label="Amount paid"
                          value={
