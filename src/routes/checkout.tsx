...
export default function CheckoutPage() {
  ...
  const defaultAddress = useMemo(
    () =>
      (currentUser
        ? getCustomerDefaultAddress(state, currentUser.id)
        : undefined) ??
  ...
  const [shippingSelections, setShippingSelections] = useState<Record<string, SellerShippingSelection>>({});

  const grouped = useMemo(
    () => (cartActorId ? getCartGroups(state, cartActorId) : []),
    [cartActorId, state.cartsByUserId, state.inventory],
  );
  const sellerSlugs = useMemo(
    () => grouped.map((group) => group.seller.slug),
    [grouped],
  );
  const cartQuantity = useMemo(
    () => (cartActorId ? getCartQuantity(state, cartActorId) : 0),
    [cartActorId, state.cartsByUserId],
  );
  ...
  const shippingPriceMap = useMemo(
    () =>
      Object.fromEntries(
        Object.values(shippingSelections).map((selection) => [selection.sellerSlug, selection.price]),
      ),
    [shippingSelections],
  );
        ...
        <div className="container mx-auto max-w-2xl px-4 py-10 sm:py-12">
          <div className="rounded-[28px] bg-card p-6 text-center shadow-[var(--shadow-premium)]">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Seller and admin sessions cannot place marketplace orders. Switch to a customer account or continue shopping as a signed-out guest.
            </p>
    ...
    <PageLayout>
      {createdOrder && createdPayment ? (
        ...
        <section className="container mx-auto px-4 py-6 sm:py-8">
            <h1 className="text-[2rem] font-black tracking-tight sm:text-3xl md:text-4xl">Secure checkout</h1>
            <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
              COD orders move straight into seller fulfilment, but SpareKart verifies the collected cash after delivery before releasing seller payouts. Manual methods stay inventory-safe until admin approval.
            </p>
            {!currentUser ? (
              <div className="mt-4 rounded-[20px] border border-info/15 bg-info/5 px-4 py-3 text-sm text-muted-foreground">
                You are checking out as a guest. We will use your phone and optional email for order tracking, proof submission, and delivered-order reviews.
              </div>
            ) : null}

            <div className="mt-6 grid max-w-3xl grid-cols-2 gap-3 sm:mt-8 sm:flex sm:items-center sm:gap-2">
              {steps.map((entry, index) => (
                <div key={entry.key} className="flex items-center gap-2 rounded-2xl bg-surface p-3 shadow-[var(--shadow-soft)] sm:flex-1 sm:rounded-none sm:bg-transparent sm:p-0 sm:shadow-none">
                  <div className={`grid h-10 w-10 place-items-center rounded-full text-sm font-bold transition-colors ${index < step ? "bg-success text-success-foreground" : index === step ? "gradient-accent text-primary" : "bg-surface-2 text-muted-foreground"}`}>
                    {index < step ? <Check className="h-5 w-5" /> : <entry.Icon className="h-4 w-4" />}
                  </div>
                  <div className="min-w-0">
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Step {index + 1}</div>
                    <div className={`text-[13px] font-bold sm:text-sm ${index <= step ? "text-foreground" : "text-muted-foreground"}`}>{entry.label}</div>
                  </div>
                  {index < steps.length - 1 && <div className={`hidden h-0.5 flex-1 sm:block ${index < step ? "bg-success" : "bg-border"}`} />}
                </div>
              ))}
            </div>
              ...
              <aside className="lg:sticky lg:top-32 lg:self-start">
                <div className="rounded-[26px] bg-card p-5 shadow-[var(--shadow-premium)] sm:rounded-[30px] sm:p-6">
                    ...
                    {grouped.flatMap((group) =>
                      group.items.map(({ line, product }) => (
                        <div key={product.id} className="flex gap-3 text-sm">
                          <OptimizedImage src={product.images[0]} alt={product.title} width={48} height={48} className="h-12 w-12 rounded-lg object-cover" />
                  ...
                  <dl className="mt-5 space-y-2 border-t border-border pt-5 text-sm">
                    <div className="flex justify-between"><dt className="text-muted-foreground">Subtotal</dt><dd className="font-semibold tabular-nums">{formatPKR(totals.subtotal)}</dd></div>
                    <div className="flex justify-between"><dt className="text-muted-foreground">Shipping</dt><dd className="font-semibold tabular-nums">{totals.shipping === 0 ? <span className="text-success">FREE</span> : formatPKR(totals.shipping)}</dd></div>
                    <div className="flex justify-between text-success"><dt>{totals.appliedCoupon ? `Coupon (${totals.appliedCoupon.code})` : "Coupon savings"}</dt><dd className="font-semibold tabular-nums">– {formatPKR(totals.discount)}</dd></div>
                    <div className="mt-2 flex justify-between border-t border-border pt-3"><dt className="font-black">Total</dt><dd className="text-xl font-black tabular-nums">{formatPKR(totals.total)}</dd></div>
                  </dl>
                  <div className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
                    <ShieldCheck className="mt-0.5 h-4 w-4 shrink-0 text-success" />
                    <span>Manual payments confirm after admin approval. COD orders can be fulfilled immediately, but their cash settlement is still verified by SpareKart after delivery.</span>
                  </div>
                </div>
              </aside>
            </div>
...
function CheckoutSuccess({
  ...
  return (
    <div className="container mx-auto max-w-2xl px-4 py-14 text-center sm:py-20">
      ...
      <h1 className="mt-6 text-[2rem] font-black tracking-tight sm:text-3xl md:text-4xl">{title}</h1>
      ...
      <div className="mt-8 rounded-[24px] bg-surface p-5 text-left shadow-[var(--shadow-soft)] sm:rounded-[28px] sm:p-6">
        ...
        <div className="mt-4 space-y-2 text-sm">
          <div className="flex justify-between"><span className="text-muted-foreground">Payment method</span><span className="font-semibold">{payment.instructionsSnapshot?.label ?? payment.method.replaceAll("_", " ")}</span></div>
          {order.appliedCoupon ? (
          ...
          <div className="flex justify-between"><span className="text-muted-foreground">Total</span><span className="font-semibold tabular-nums">{formatPKR(order.totals.total)}</span></div>
          <div className="flex justify-between"><span className="text-muted-foreground">Sellers</span><span className="font-semibold tabular-nums">{new Set(order.items.map((item) => item.sellerSlug)).size}</span></div>
        </div>
      </div>
      <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
        ...
        <Link href="/" className="flex h-11 items-center justify-center rounded-xl bg-surface px-6 text-sm font-semibold shadow-[var(--shadow-soft)]">
          Back to home
        </Link>
      </div>
    </div>
...
function AddressStep({
  ...
  const updateField = <Key extends keyof ShippingAddress>(key: Key, fieldValue: ShippingAddress[Key]) =>
    ...
    <div className="space-y-4 rounded-[24px] bg-card p-5 shadow-[var(--shadow-soft)] sm:rounded-[28px] sm:p-6">
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <Field label="Full name">
          <input value={value.fullName} onChange={(event) => updateField("fullName", event.target.value)} className="h-11 w-full rounded-xl bg-surface px-3 text-sm shadow-[var(--shadow-soft)] focus:outline-none focus:ring-2 focus:ring-accent/30" />
        </Field>
        <Field label="Phone">
          <input value={value.phone} onChange={(event) => updateField("phone", event.target.value)} className="h-11 w-full rounded-xl bg-surface px-3 text-sm shadow-[var(--shadow-soft)] focus:outline-none focus:ring-2 focus:ring-accent/30" />
        </Field>
      </div>
      {isGuestCheckout ? (
      ...
      <Field label="Address line">
        <input value={value.addressLine} onChange={(event) => updateField("addressLine", event.target.value)} className="h-11 w-full rounded-xl bg-surface px-3 text-sm shadow-[var(--shadow-soft)] focus:outline-none focus:ring-2 focus:ring-accent/30" />
      </Field>
      <div className="grid gap-3 sm:grid-cols-3">
        <Field label="City">
          <input value={value.city} onChange={(event) => updateField("city", event.target.value)} className="h-11 w-full rounded-xl bg-surface px-3 text-sm shadow-[var(--shadow-soft)] focus:outline-none focus:ring-2 focus:ring-accent/30" />
        </Field>
        <Field label="Province">
          <input value={value.province} onChange={(event) => updateField("province", event.target.value)} className="h-11 w-full rounded-xl bg-surface px-3 text-sm shadow-[var(--shadow-soft)] focus:outline-none focus:ring-2 focus:ring-accent/30" />
        </Field>
        <Field label="Postal code">
          <input value={value.postalCode} onChange={(event) => updateField("postalCode", event.target.value)} className="h-11 w-full rounded-xl bg-surface px-3 text-sm shadow-[var(--shadow-soft)] focus:outline-none focus:ring-2 focus:ring-accent/30" />
        </Field>
      </div>
    </div>
  );
...
function ShippingStep({
  ...
  return (
    <div className="space-y-4">
      ...
      {groups.map((group) => {
        const selected = selections[group.seller.slug] ?? createShippingSelection(group.seller.slug, "STANDARD");
        return (
          <div key={group.seller.slug} className="rounded-[24px] bg-card p-4 shadow-[var(--shadow-soft)] sm:rounded-[26px] sm:p-5">
            <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-1.5 text-sm font-bold">{group.seller.name} {group.seller.verified && <BadgeCheck className="h-3.5 w-3.5 text-info" />}</div>
                  <div className="text-xs text-muted-foreground">{group.items.length} items · ships from {group.seller.city}</div>
                </div>
              </div>
            </div>
            <div className="space-y-2">
              {(["STANDARD", "EXPRESS"] as ShippingOptionId[]).map((optionId) => {
                ...
                return (
                  <label key={optionId} className={`flex cursor-pointer flex-col gap-3 rounded-xl p-4 transition-all sm:flex-row sm:items-center sm:justify-between ${selected.optionId === optionId ? "border-2 border-accent bg-accent-soft shadow-[var(--shadow-soft)]" : "bg-surface shadow-[var(--shadow-soft)] hover:bg-background"}`}>
                    <div className="flex items-center gap-3">
                      <input type="radio" name={`ship-${group.seller.slug}`} checked={selected.optionId === optionId} onChange={() => onSelect(group.seller.slug, optionId)} className="h-4 w-4 accent-[oklch(0.72_0.19_50)]" />
                      <div>
      ...
      <div className="rounded-[24px] bg-card p-5 shadow-[var(--shadow-soft)] sm:rounded-[28px] sm:p-6">
        ...
        <div className="mt-4 grid gap-3 sm:grid-cols-2">
          {paymentOptions.map((option) => (
            <label key={option.id} className={`relative flex cursor-pointer items-start gap-3 rounded-2xl p-4 transition-all ${paymentMethod === option.id ? "border-2 border-accent bg-accent-soft shadow-[var(--shadow-glow)]" : "bg-surface shadow-[var(--shadow-soft)] hover:bg-background"}`}>
              <input type="radio" name="pay" checked={paymentMethod === option.id} onChange={() => setPaymentMethod(option.id)} className="mt-1 h-4 w-4 accent-[oklch(0.72_0.19_50)]" />
              <div className="flex-1">
                <div className="flex items-center gap-2">
                  <option.Icon className="h-4 w-4" />
                  <span className="font-bold text-sm">{option.label}</span>
                  {option.badge && <span className="rounded bg-accent px-1.5 py-0.5 text-[10px] font-bold text-primary">{option.badge}</span>}
                </div>
      ...
      {paymentMethod !== "COD" && manualInstructions && (
        <>
          <div className="space-y-5 rounded-[24px] bg-card p-5 shadow-[var(--shadow-soft)] sm:rounded-[28px] sm:p-6">
            <div>
              <h3 className="flex items-center gap-2 text-lg font-black">
                <ShieldCheck className="h-5 w-5 text-info" /> {manualInstructions.label} instructions
              </h3>
            ...
            <div className="space-y-2 rounded-2xl bg-surface p-4 text-sm shadow-[var(--shadow-soft)] sm:p-5">
              ...
              {manualInstructions.bankName && <InstructionRow label="Bank" value={manualInstructions.bankName} />}
              {manualInstructions.accountNumber && <InstructionRow label="Account number" value={manualInstructions.accountNumber} monospace />}
              {manualInstructions.iban && <InstructionRow label="IBAN" value={manualInstructions.iban} monospace />}
              {manualInstructions.walletNumber && <InstructionRow label="Wallet number" value={manualInstructions.walletNumber} monospace />}
            ...
            <div className="rounded-[24px] bg-card p-5 shadow-[var(--shadow-soft)] sm:rounded-[28px] sm:p-6">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-sm text-muted-foreground">{proofDraft.screenshotName} · Ref {proofDraft.transactionReference}</div>
                </div>
                <button onClick={() => setProofDraft(undefined)} className="text-sm font-semibold text-accent hover:underline">
                  Replace proof
                </button>
              </div>
            </div>
          ...
          <div className="rounded-[24px] bg-accent-soft/80 p-4 text-sm text-muted-foreground shadow-[var(--shadow-soft)]">
            <strong className="text-foreground">Flexible submission:</strong> you can place the order now without proof, then upload or resubmit it later from your order tracking screen.
          </div>
        </>
      )}
    </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-[24px] bg-card p-4 text-sm shadow-[var(--shadow-soft)] sm:rounded-[26px] sm:p-5">
        ...
        <div className="text-muted-foreground">
          {shippingAddress.fullName} · {shippingAddress.phone} · {shippingAddress.addressLine}, {shippingAddress.city} {shippingAddress.postalCode}, {shippingAddress.province}
        </div>
        {isGuestCheckout ? (
      ...
      <div className="rounded-[24px] bg-card p-4 text-sm shadow-[var(--shadow-soft)] sm:rounded-[26px] sm:p-5">
        ...
        {paymentMethod !== "COD" && (
          <div className="mt-2 text-xs text-muted-foreground">
            {paymentProofReady ? "Proof will be submitted with this order." : "Proof can be submitted later from order tracking if you place the order now."}
          </div>
        )}
      </div>
      {groups.map((group) => {
        const subTotal = group.items.reduce((sum, item) => sum + item.product.price * item.line.qty, 0);
          ...
          <div key={group.seller.slug} className="rounded-[24px] bg-card p-4 shadow-[var(--shadow-soft)] sm:rounded-[26px] sm:p-5">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-border pb-3">
              <div className="min-w-0">
                ...
                <div className="text-xs text-muted-foreground">{group.items.length} items · {shipping?.label ?? "Standard delivery"}</div>
              </div>
            ...
            <div className="mt-3 space-y-2 text-sm">
              {group.items.map(({ line, product }) => (
                <div key={product.id} className="flex gap-3 justify-between">
                  <span className="min-w-0 text-muted-foreground">{product.title} × {line.qty}</span>
                  <span className="shrink-0 tabular-nums">{formatPKR(product.price * line.qty)}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="rounded-[24px] bg-card p-4 shadow-[var(--shadow-soft)] sm:rounded-[26px] sm:p-5">
        <div className="space-y-2 text-sm">
          ...
          <div className="flex justify-between">
            ...
            <span className="tabular-nums">{totals.shipping === 0 ? "FREE" : formatPKR(totals.shipping)}</span>
          </div>
          <div className="flex justify-between text-success">
            <span>{totals.appliedCoupon ? `Coupon (${totals.appliedCoupon.code})` : "Coupon savings"}</span>
