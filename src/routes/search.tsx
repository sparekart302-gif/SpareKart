  ...
  return (
    <PageLayout>
      ...
      <section className="container mx-auto px-4 pb-10 sm:pb-12 md:pb-16">
        <form onSubmit={handleSubmit} className="flex max-w-2xl flex-col gap-2 rounded-[22px] bg-surface p-2 shadow-[var(--shadow-soft)] focus-within:ring-2 focus-within:ring-accent/30 sm:flex-row sm:items-center sm:rounded-[24px]">
          <select
          ...
          <div className="flex items-center rounded-xl bg-background/70 sm:flex-1 sm:bg-transparent">
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search parts, brands, SKUs…" className="h-11 flex-1 bg-transparent px-3 text-sm focus:outline-none" />
          </div>
          <button type="submit" className="h-11 rounded-xl gradient-accent px-5 text-sm font-bold text-primary sm:px-6">Search</button>
        </form>
        ...
        <div className="mt-5 flex flex-wrap items-center justify-between gap-3 sm:mt-6">
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              <span className="font-semibold text-foreground tabular-nums">{results.length}</span> products found
              {activeCategory ? ` in ${activeCategory.name}` : ""}
              {results.length === 0 && (queryFromUrl || activeCategory) && " — showing popular alternatives"}
            </p>
          </div>
          {results.length === 0 && (queryFromUrl || activeCategory) && (
            <div className="flex items-center gap-2 text-xs text-accent font-semibold">
              ...
              <button type="button" onClick={() => router.push("/search?q=brake")} className="underline">brake</button>,
              <button type="button" onClick={() => router.push("/search?q=oil%20filter")} className="underline">oil filter</button>?
            </div>
          )}
        </div>
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {(queryFromUrl || activeCategory) && (
            <Link to="/search" className="rounded-full bg-background px-3 py-1.5 text-xs font-semibold text-muted-foreground shadow-[var(--shadow-soft)] transition-colors hover:bg-surface hover:text-foreground">
              Clear search
            </Link>
          )}
        </div>
