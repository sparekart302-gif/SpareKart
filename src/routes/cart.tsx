      ...
      <section className="container mx-auto px-4 pb-10 sm:pb-12 md:pb-16">
        <div className="mb-6 flex flex-col gap-2 sm:mb-8 sm:flex-row sm:items-center sm:justify-between">
          ...
          <span className="text-sm text-muted-foreground tabular-nums">
            {cartQuantity} items · from {grouped.length} sellers
          </span>
        </div>
        ...
        {cartQuantity === 0 ? (
          <div className="gradient-surface rounded-[26px] p-10 text-center shadow-[var(--shadow-soft)] sm:rounded-[30px] sm:p-16">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Find genuine parts for your car from Pakistan's top verified sellers.
            </p>
            <Link
              to="/shop"
              className="mt-6 inline-flex h-11 items-center gap-2 rounded-xl gradient-accent px-6 text-sm font-bold text-primary"
            >
              ...
              {grouped.map((group) => {
                ...
                return (
                  <div
                    ...
                    className="overflow-hidden rounded-[24px] bg-card shadow-[var(--shadow-soft)] sm:rounded-[28px]"
                  >
                    <div className="flex flex-col gap-3 bg-surface px-4 py-4 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                      <Link
                        to="/seller/$slug"
                        params={{ slug: group.seller.slug }}
                      ...
                      >
                        ...
                        />
                        <div>
                          <div className="flex items-center gap-1.5 text-sm font-bold group-hover:text-accent">
                            {group.seller.name}{" "}
                          ...
                          <div className="flex items-center gap-2 text-xs text-muted-foreground">
                            <Truck className="h-3 w-3" /> Ships from {group.seller.city}
                          </div>
                        </div>
                      </Link>
                    ...
                    {group.items.map(({ line, product, availableStock }) => (
                      <div
                        key={product.id}
                        className="flex flex-col gap-3 border-t border-border/70 p-4 first:border-t-0 sm:flex-row sm:gap-4 sm:p-5"
                      >
                        <Link
                          to="/product/$slug"
                        ...
                        <div className="flex min-w-0 flex-1 flex-col">
                          <Link
                            to="/product/$slug"
                            params={{ slug: product.slug }}
                            className="line-clamp-2 font-bold hover:text-accent"
                          >
                            {product.title}
                          </Link>
                          ...
                          <div
                            className={`mt-1 text-xs font-semibold ${availableStock > 0 ? "text-success" : "text-destructive"}`}
                          >
                            {availableStock > 0
                              ? `${availableStock} available for checkout`
                          ...
                          <div className="mt-3 flex flex-wrap items-center justify-between gap-3 sm:mt-auto sm:pt-3">
                            <div className="inline-flex items-center rounded-xl bg-surface shadow-[var(--shadow-soft)]">
                              <button
                                onClick={() => void updateCartQty(product.id, line.qty - 1)}
                                className="grid h-9 w-9 place-items-center hover:bg-background"
                              >
                        ...
                        <div className="shrink-0 text-left sm:text-right">
                          ...
                          {product.comparePrice && (
                            <div className="text-xs text-muted-foreground line-through tabular-nums">
                              {formatPKR(product.comparePrice * line.qty)}
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <aside className="space-y-4 lg:sticky lg:top-32 lg:self-start">
              <div className="rounded-[26px] bg-card p-5 shadow-[var(--shadow-premium)] sm:rounded-[30px] sm:p-6">
                ...
                {!currentUser ? (
                  <div className="mt-3 rounded-2xl border border-info/15 bg-info/5 px-3 py-3 text-xs text-muted-foreground">
                    Guest checkout is available. You can place the order without signing in and
                    track it later using your order number and phone or email.
                  </div>
                ) : null}
                <dl className="mt-4 space-y-2.5 text-sm">
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Subtotal ({cartQuantity} items)</dt>
                    <dd className="font-semibold tabular-nums">{formatPKR(totals.subtotal)}</dd>
                  </div>
                  <div className="flex justify-between">
                    <dt className="text-muted-foreground">Shipping ({grouped.length} sellers)</dt>
                ...
                <Link
                  to="/checkout"
                  className="mt-5 flex h-12 w-full items-center justify-center gap-2 rounded-xl gradient-accent text-sm font-bold text-primary hover:opacity-95"
                >
                ...
                <div className="mt-4 flex items-start gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    Secure marketplace checkout. COD stays available, while manual payments are
                    confirmed only after admin verification.
                  </span>
                </div>
              </div>
              ...
              {totals.subtotal < freeShippingThreshold && (
                <div className="rounded-[22px] bg-accent-soft/80 p-4 text-sm shadow-[var(--shadow-soft)] sm:rounded-[24px]">
                  <strong className="text-foreground">Almost there!</strong> Add{" "}
                  <span className="font-bold tabular-nums">
                    {formatPKR(freeShippingThreshold - totals.subtotal)}
                  </span>{" "}
                  more to unlock <strong>FREE shipping</strong> across all sellers.
                </div>
              )}
            </aside>
          </div>
