    ...
    <AdminScopeGate scope="audit" currentUser={currentUser}>
      <div className="space-y-4">
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          <AdminCompactStat
            label="Events"
            value={String(state.auditTrail.length)}
            helper="All recorded actions"
          />
          <AdminCompactStat
            label="Today"
            value={String(
              state.auditTrail.filter((entry) =>
                entry.createdAt.startsWith(new Date().toISOString().slice(0, 10)),
              ).length,
            )}
            helper="Events in current UTC day"
          />
          <AdminCompactStat
            label="Admins"
            value={String(new Set(state.auditTrail.map((entry) => entry.actorUserId)).size)}
            helper="Distinct actors"
          />
          <AdminCompactStat
            label="Filtered"
        ...
        <OperationsWorkspace>
          <OperationsPanel
            title="Audit stream"
            description="Filter high-volume action logs, then inspect event metadata in the side panel."
          >
            <OperationsToolbar>
              <OperationsSearch
                value={query}
            ...
            {filteredLogs.length === 0 ? (
              <div className="p-3">
                <AdminEmptyState
                  ...
                  body="Try adjusting your search or action filter."
                />
              </div>
            ) : null}
          ...
          <OperationsDetailPanel
            ...
            subtitle={
              selectedLog
                ? `Recorded ${new Date(selectedLog.createdAt).toLocaleString()}`
                : "Choose an event from the audit stream."
            }
            meta={
              selectedLog ? (
                <AdminPill tone="info">{selectedLog.actorRole.replaceAll("_", " ")}</AdminPill>
              ) : null
            }
            empty={
              <AdminEmptyState
                title="Select an event"
                body="Pick an audit row to inspect actor, timestamp, and note details."
              />
            }
          >
            {selectedLog ? (
              <div className="space-y-3">
                <div className="rounded-[14px] border border-border/70 bg-background p-3">
                  ...
                  <OperationsKeyValue
                    label="Email"
                    value={selectedActor?.email ?? "Automated event"}
                  />
                  <OperationsKeyValue
                    label="Role"
                    value={selectedLog.actorRole.replaceAll("_", " ")}
                  />
                ...
                <div className="rounded-[14px] border border-border/70 bg-background p-3">
                  ...
                  <p className="mt-2 text-sm leading-6 text-foreground/80">
                    {selectedLog.note ?? "No note was stored with this audit event."}
                  </p>
                </div>
              </div>
            ) : null}
