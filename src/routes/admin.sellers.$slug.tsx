  ...
  return (
    <div className="space-y-3">
      ...
      <section className="grid gap-2.5 sm:grid-cols-2 xl:grid-cols-4">
        <AdminCompactStat label="Revenue" value={formatPKR(revenue)} helper="Seller-attributed value" tone="success" />
        <AdminCompactStat label="Products" value={String(products.length)} helper="Linked catalog items" />
        <AdminCompactStat label="Open orders" value={String(pendingOrders.length)} helper="Still in fulfilment flow" tone="warning" />
        <AdminCompactStat label="Platform commission" value={formatPKR(commissionSummary.totalCommission)} helper="Marketplace share" />
      </section>

      <section className="grid gap-4 xl:grid-cols-[0.82fr_1.18fr]">
        <div className="space-y-4">
          <AdminPanel title="Governance summary" description="Operational and commercial controls for this seller record.">
            <div className="divide-y divide-border/60">
              <AdminKeyValue label="Joined" value={seller.joined} />
              <AdminKeyValue label="Response time" value={seller.responseTime} />
              <AdminKeyValue label="Rating" value={`${seller.rating.toFixed(1)} / 5 from ${seller.reviewCount} reviews`} />
              <AdminKeyValue label="Commission rate" value={`${seller.commissionRate}%`} />
              <AdminKeyValue label="Payout hold" value={seller.payoutHold ? "Enabled" : "Not active"} />
              <AdminKeyValue label="Last active" value={seller.lastActiveAt ? new Date(seller.lastActiveAt).toLocaleString() : "No recent session"} />
            </div>

            {seller.approvalNote ? (
              <div className="mt-4 rounded-[16px] border border-border/60 px-3.5 py-3 text-sm text-muted-foreground">
                <span className="font-semibold text-foreground">Approval note:</span> {seller.approvalNote}
              </div>
            ) : null}
          ...
          <AdminPanel title="Policies and permissions">
            <div className="divide-y divide-border/60">
              ...
              <AdminKeyValue label="Feature products" value={seller.permissions.canFeatureProducts ? "Enabled" : "Disabled"} />
              <AdminKeyValue label="Run campaigns" value={seller.permissions.canRunCampaigns ? "Enabled" : "Disabled"} />
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              ...
              <ReviewChip
                ...
                active={Boolean(seller.policies.returns && seller.policies.shipping && seller.policies.warranty)}
              />
              <ReviewChip label="Branding approved" active={seller.status === "ACTIVE" && seller.verified} />
            </div>
          </AdminPanel>
        </div>

        <div className="space-y-4">
          <AdminPanel title="Operational activity" description="Latest marketplace orders routed to this seller.">
            {orders.length === 0 ? (
              <div className="text-sm text-muted-foreground">No orders routed to this seller yet.</div>
            ) : (
              <div className="divide-y divide-border/60">
                      ...
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 text-xs text-muted-foreground">
                            {new Date(order.createdAt).toLocaleString()} · {sellerItems.length} seller items
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm font-black text-foreground">
                            {formatPKR(sellerItems.reduce((sum, item) => sum + item.unitPrice * item.quantity, 0))}
                          </div>
          ...
          <AdminPanel title="Commission ledger" description="Order-wise marketplace commission tracked for this seller.">
            {commissionRows.length === 0 ? (
              <div className="text-sm text-muted-foreground">No commission ledger rows yet for this seller.</div>
            ) : (
              <div className="divide-y divide-border/60">
                  ...
                  <div key={`${row.orderId}-${row.sellerSlug}`} className="py-3 first:pt-0 last:pb-0">
                    <div className="flex flex-wrap items-start justify-between gap-3">
                      ...
                      <div className="text-right">
                        ...
                        <div className="mt-1 text-[11px] text-muted-foreground">{row.commissionRate}% commission</div>
                      </div>
                    </div>
                  </div>
                ))}
          ...
          <AdminPanel title="Catalog overview" description="Products currently live under this seller record.">
            {products.length === 0 ? (
              <div className="text-sm text-muted-foreground">No active products linked to this seller.</div>
            ) : (
              <div className="divide-y divide-border/60">
                {products.slice(0, 6).map((product) => (
                  ...
                  >
                    <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-2xl border border-border/60">
                      <OptimizedImage src={product.images[0]} alt={product.title} fill className="object-cover" sizes="56px" />
                    </div>
                    <div className="min-w-0 flex-1">
                      <div className="line-clamp-2 text-sm font-bold text-foreground">{product.title}</div>
                      <div className="mt-1 text-xs text-muted-foreground">
