    ...
    <PageLayout>
      <section className="container mx-auto px-4 pb-8 sm:pb-10">
        <div className="grid items-center gap-5 rounded-[28px] gradient-hero px-4 py-8 text-primary-foreground shadow-[var(--shadow-premium)] sm:px-6 sm:py-12 lg:grid-cols-2 lg:gap-10 lg:rounded-[36px] lg:px-8 lg:py-16">
          <div>
            ...
            <h1 className="mt-4 text-[2rem] font-black tracking-tight text-balance sm:text-4xl md:text-5xl lg:text-6xl">Grow your auto parts business with SpareKart.</h1>
            <p className="mt-4 max-w-xl text-sm opacity-85 sm:mt-5 sm:text-lg">Reach thousands of verified buyers across Pakistan. Zero monthly fees, full marketplace support, and fast payouts.</p>
            <div className="mt-6 flex flex-col gap-3 sm:mt-7 sm:flex-row">
              <Link to="/register" className="flex h-11 items-center justify-center gap-2 rounded-xl gradient-accent px-6 text-sm font-bold text-primary hover:opacity-95 sm:h-12">Apply to sell <ArrowRight className="h-4 w-4" /></Link>
              <Link to="/sellers" className="flex h-11 items-center justify-center gap-2 rounded-xl bg-white/10 px-6 text-sm font-bold backdrop-blur hover:bg-white/15 sm:h-12">Browse stores</Link>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-3 sm:gap-4">
            {[
              ...
              <div key={i} className="rounded-[20px] bg-white/10 p-4 backdrop-blur sm:rounded-[24px] sm:p-5">
      ...
      <section className="container mx-auto px-4 py-10 sm:py-12 md:py-16">
        <div className="mx-auto max-w-2xl text-center">
          ...
          <h2 className="mt-2 text-[1.9rem] font-black sm:text-3xl md:text-4xl">Start selling in 3 steps</h2>
        </div>
        <div className="mt-8 grid gap-4 sm:mt-10 sm:gap-5 md:grid-cols-3">
          {[
            ...
            <div key={s.n} className="rounded-[24px] bg-surface p-5 shadow-[var(--shadow-soft)] transition-all hover:-translate-y-0.5 hover:shadow-[var(--shadow-elevated)] sm:rounded-[26px] sm:p-6">
        ...
        <div className="mx-auto mt-10 max-w-2xl rounded-[26px] bg-card p-5 shadow-[var(--shadow-premium)] sm:mt-12 sm:rounded-[30px] sm:p-6">
          ...
          <Link to="/register" className="mt-6 flex h-11 w-full items-center justify-center gap-2 rounded-xl gradient-accent text-sm font-bold text-primary sm:h-12">Apply to become a seller <ArrowRight className="h-4 w-4" /></Link>
        </div>
      </section>
    </PageLayout>
  );
